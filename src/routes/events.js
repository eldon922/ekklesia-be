  ...
  try {
    const result = await pool.query(`
      SELECT
        ...
        e.is_finished, e.created_at, e.updated_at,
...
router.get("/:id", param("id").isInt(), async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      data: { ...sanitize(event.rows[0]), ...stats.rows[0] },
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: "Server error" });
...
router.post("/:id/verify-password", param("id").isInt(), async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT password_hash FROM events WHERE id = $1",
      [req.params.id],
    );
...
router.patch("/:id/finish", param("id").isInt(), async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `UPDATE events SET is_finished = TRUE, updated_at = NOW() WHERE id = $1 RETURNING *`,
      [req.params.id],
    );
    if (result.rows.length === 0)
      return res
    ...
    if (io)
      io.to(`event:${req.params.id}`).emit("event:finished", {
...
router.patch("/:id/restart", param("id").isInt(), async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `UPDATE events SET is_finished = FALSE, updated_at = NOW() WHERE id = $1 RETURNING *`,
      [req.params.id],
    );
    if (result.rows.length === 0)
      return res
    ...
    const io = req.app.get("io");
    if (io)
      io.to(`event:${req.params.id}`).emit("event:restarted", {
        eventId: parseInt(req.params.id),
      });
